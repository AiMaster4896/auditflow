...
let allClients = [];
let activeFilters = { search: "", status: "", branch: "", fye: "", staff: "", partner: "" };
let clientsByStaff = {};
...
function populateStaffPartnerFilters() {
  ...
  const options = staffAndPartnerOptions.map((m) => `<option value="${m.user_id}">${m.profiles?.display_name || m.profiles?.email}</option>`).join("");
  staffSelect.innerHTML = `<option value="">All Staff</option>${options}`;
  partnerSelect.innerHTML = `<option value="">All Partners</option>${options}`;
}
...
async function loadAndRenderTable() {
  ...
  const [{ data, error }, { data: openWOs }, { data: allWOs }, { data: members }] = await Promise.all([
    supabase
      .from("clients")
          ...
          .map(
            (c) => `<tr class="clickable-row" data-row-client="${c.id}">
              ...
              <td class="row-actions">
                ...
                ${isFirmAdmin() ? `<button class="icon-btn icon-btn-delete" data-delete="${c.id}" data-name="${c.legal_name}" title="Delete">${DELETE_ICON}</button>` : ""}
              </td>
            </tr>`
          )
          .join("")}
...
async function openClientDetailsModal(client) {
  ...
  const [{ data: contact }, { data: workOrders }] = await Promise.all([
    ...
    supabase
      .from("work_orders")
      .select("id, order_type, financial_year_end, deadline_date, status, description, professional_fee, ope, budget_fee, client_id, assigned_user_id, partner_user_id, assigned:profiles!assigned_user_id(display_name, email), partner:profiles!partner_user_id(display_name, email)")
    ...
    <div class="modal-card">
      <div class="modal-header">
        <div>
          ...
          <p class="modal-subtitle">${isEdit ? (isFirmAdmin() ? "Update this client's information" : "Changes will be sent to your admin for approval") : "Enter information for the new client"}</p>
        </div>
        <button type="button" id="modal-close-btn" class="modal-close" aria-label="Close">&times;</button>
      </div>
      <form id="client-form">
        ...
        <div class="modal-actions">
          ...
          <button type="submit" class="btn-dark">${isEdit ? (isFirmAdmin() ? "Save Changes" : "Submit for Approval") : "Add Client"}</button>
        </div>
      </form>
    </div>
  `;
