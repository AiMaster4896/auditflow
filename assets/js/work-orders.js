...
export const ORDER_TYPE_LABELS = {
  ...
  client_update: "Change of Company Details",
};
...
const MANUAL_ORDER_TYPES = Object.fromEntries(Object.entries(ORDER_TYPE_LABELS).filter(([k]) => k !== "client_update"));
...
let woFilters = { client: "", type: "", status: "", staff: "", partner: "" };
...
export async function renderWorkOrders(el) {
  ...
  document.getElementById("wo-filter-staff").addEventListener("change", (e) => { woFilters.staff = e.target.value; renderFilteredTable(); });
  document.getElementById("wo-filter-partner").addEventListener("change", (e) => { woFilters.partner = e.target.value; renderFilteredTable(); });
...
async function loadAndRenderAll() {
  ...
  const { data, error } = await supabase
    .from("work_orders")
    .select("id, order_type, financial_year_end, deadline_date, status, description, professional_fee, ope, budget_fee, client_id, assigned_user_id, partner_user_id, clients(legal_name), assigned:profiles!assigned_user_id(display_name, email), partner:profiles!partner_user_id(display_name, email)")
  ...
  container.innerHTML = `
    <table class="data-table">
      <thead><tr>
        ...
        <th>Type</th><th>Financial Year</th><th>Assigned Staff</th><th>Partner</th><th>Deadline</th><th>Budget Fee</th><th>Status</th><th>Remark</th><th>Actions</th>
      </tr></thead>
      <tbody>
        ${orders
          .map((wo) => {
            ...
            const canUpdateStatus = !isClientUpdate && (admin || wo.assigned_user_id === selfId);
            ...
            return `<tr class="clickable-row" data-wo-row="${wo.id}">
              ...
              <td>
                ${canUpdateStatus
                  ...
                  : `<span class="status-badge ${statusBadgeClass}">${STATUS_LABELS[wo.status]}</span>`
                }
              </td>
...
function showWorkOrderDetailsModal(wo) {
  ...
  backdrop.innerHTML = `
    <div class="modal-card" style="max-width:560px;">
      <div class="modal-header">
        <div>
          <h2 class="modal-title">${ORDER_TYPE_LABELS[wo.order_type] || wo.order_type}</h2>
          <p class="modal-subtitle">${wo.clients?.legal_name || "Work order details"}</p>
        </div>
      ...
      <div class="modal-grid">
        ...
        <div><label class="option-label">Type</label><div>${ORDER_TYPE_LABELS[wo.order_type] || wo.order_type}</div></div>
        ...
        <div><label class="option-label">Assigned Staff</label><div>${wo.assigned?.display_name || wo.assigned?.email || "-"}</div></div>
        <div><label class="option-label">Deadline</label><div>${isoToDMY(wo.deadline_date) || "-"}</div></div>
        ...
        <div><label class="option-label">Status</label><div>${STATUS_LABELS[wo.status] || wo.status}</div></div>
...
export async function openWorkOrderModal({ clientId, existing, onSaved } = {}) {
  ...
  if (!allStaffForAssign.length) {
    const { data: members } = await supabase.from("organisation_members").select("user_id, profiles(display_name, email)").eq("organisation_id", orgId).eq("status", "active").eq("is_working_staff", true);
    allStaffForAssign = members || [];
  }
      ...
      <form id="wo-form">
        ...
        <label style="display:block;margin-bottom:14px;">Type
          <select id="wo-type" required style="width:100%;margin-top:6px;padding:9px 12px;border:1px solid var(--gray-200);border-radius:8px;font-size:14px;">
            ${Object.entries(MANUAL_ORDER_TYPES).map(([k, v]) => `<option value="${k}" ${existing?.order_type === k ? "selected" : ""}>${v}</option>`).join("")}
          </select>
        </label>
        ...
        <label style="display:block;margin-bottom:14px;">Partner
          <select id="wo-partner" required style="width:100%;margin-top:6px;padding:9px 12px;border:1px solid var(--gray-200);border-radius:8px;font-size:14px;">
            ...
            ${allStaffForAssign.map((m) => `<option value="${m.user_id}" ${existing?.partner_user_id === m.user_id ? "selected" : ""}>${m.profiles?.display_name || m.profiles?.email}</option>`).join("")}
          </select>
        </label>
