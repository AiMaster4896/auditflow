...
async function loadAndRenderUsers() {
  ...
  const { data, error } = await supabase
    ...
    .select("user_id, role, status, profiles(display_name, email, designation)")
  ...
  wrap.innerHTML = `
    <table class="data-table">
      <thead><tr><th>Name</th><th>Email</th><th>Designation</th><th>Role</th><th>Status</th><th>Actions</th></tr></thead>
      <tbody>
        ${data
          .map((m) => {
...
function openAddUserModal() {
  ...
  document.getElementById("modal-form").addEventListener("submit", async (e) => {
    ...
    if (error || data?.error) {
      errorEl.textContent = "Could not add user: " + (data?.error || error.message);
...
function openResetPasswordModal(member) {
  ...
  document.getElementById("modal-form").addEventListener("submit", async (e) => {
    ...
    if (error || data?.error) {
      errorEl.textContent = "Could not reset password: " + (data?.error || error.message);
      errorEl.classList.remove("hidden");
      submitBtn.disabled = false;
...
async function confirmDeleteUser(userId, name) {
  ...
  if (error || data?.error) {
    alert("Could not delete user: " + (data?.error || error.message));
    return;
  }
